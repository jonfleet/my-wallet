...
class ListGroup extends Component {
  renderMonth() {
    ...
    if (reportData.length !== 0 && years.length !== 0) {
      const filterByYear = reportData.filter(
  ...
  getMonthsArr(data) {
    ...
    const sortedMonths = months.sort(
      (a, b) => base.indexOf(a.name) - base.indexOf(b.name)
    );
    return sortedMonths;
  }
  ...
  render() {
    ...
    return (
      <div>
        <p className="h2">
          <span className="badge badge-secondary">{title}</span>
        </p>
        <ul className="list-group list-group-vertical pl-0 mt-2 mb-2 list-group-width">
          {items.map((item) => (
            <li
              onClick={() => onItemSelect(item[keyProperty], items)}
              className={
                item[keyProperty] === activeClass
